        ...
        private void FixedUpdate()
        {
            if (buttonContributeAllocator != null)
            {
                for (int i = 0; i < buttonContributeAllocator.elements?.Count; i++)
                {
                    ...
                    if (!buttonContributeAllocator.elements[i].interactable && contributeCosts[i] <= pickerController.networkUIPromptController.currentParticipantMaster.money && (pickerController.cachedSyncedMoneyAmount + contributeCosts[i]) <= TreasureTrove.Items.MoneyBankManager.targetMoneyAmountToStore)
                    {
                        buttonContributeAllocator.elements[i].interactable = true;
                    }
                    else if (buttonContributeAllocator.elements[i].interactable && contributeCosts[i] > pickerController.networkUIPromptController.currentParticipantMaster.money || (pickerController.cachedSyncedMoneyAmount + contributeCosts[i]) > TreasureTrove.Items.MoneyBankManager.targetMoneyAmountToStore)
                    {
