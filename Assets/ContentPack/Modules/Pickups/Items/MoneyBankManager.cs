    ...
    public class MoneyBankManager
    {
        ...
        public static uint targetMoneyAmountToStore
        {
            get
            {
                ...
                {
                    if (teamMember.body.inventory && teamMember.body.inventory.GetItemCount(TTContent.Items.MoneyBank) > 0)
                    {
                        moneyFromFirstStack += 500; //Basically count that tracks bodies that have at least one item
                    }
                }
                return (uint)(moneyFromFirstStack + ((Util.GetItemCountForTeam(TeamIndex.Player, TTContent.Items.MoneyBank.itemIndex, false, true) - (moneyFromFirstStack / 500)) * 25) * (Run.instance.stageClearCount + 1));
            }
        }
