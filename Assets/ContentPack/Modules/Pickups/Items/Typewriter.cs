    ...
    public class Typewriter : Item
    {
        ...
        internal class TypewriterBehavior : CharacterBody.ItemBehavior
        {
            ...
            private void Body_onSkillActivatedServer(GenericSkill obj)
            {
                ...
                {
                    TTLog.LogW("Function 'System.Void TurboEdition.Items.Typewriter::Body_onSkillActivatedServer() called without authority.'");
                    return;
                }
                ForceAttempt();
            ...
            private void EquipmentSlot_onServerEquipmentActivated(EquipmentSlot arg1, EquipmentIndex arg2)
            {
                ...
                if (!Util.HasEffectiveAuthority(body.networkIdentity))
                {
                    TTLog.LogW("Function 'System.Void TurboEdition.Items.Typewriter::EquipmentSlot_onServerEquipmentActivated() called without authority.'");
                    return;
                }
                ForceAttempt();
                for (int i = 0; i < stack; i++)
            ...
            private void SprintBehavior()
            {
                if (!Util.HasEffectiveAuthority(body.networkIdentity))
                {
                    TTLog.LogW("Function 'System.Void TurboEdition.Items.Typewriter::SprintBehavior() called without authority.'");
                    return;
                }
                if (body.isSprinting)
                {
