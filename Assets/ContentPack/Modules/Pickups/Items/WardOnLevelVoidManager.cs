...
namespace TreasureTrove.Items
{
    public class WardOnLevelVoidManager
    {
        ...
        private static void onCharacterLevelUp(CharacterBody obj)
        {
            ...
            int c = obj.inventory.GetItemCount(TTContent.Items.WardOnLevelVoid.itemIndex);
            if (c > 0)
            {
                obj.AddTimedBuff(TTContent.Buffs.WardOnLevelVoid, 45 + ((c - 1 * 30)));

                List<HurtBox> hurtBoxes = CollectionPool<HurtBox, List<HurtBox>>.RentCollection();
                SearchForTargets(hurtBoxes, obj);
                for (int i = 0; i < hurtBoxes.Count; i++)
                {
                    hurtBoxes[i].healthComponent.body?.AddTimedBuff(TTContent.Buffs.WardOnLevelVoid, 30 + ((c - 1 * 30) / 2));
                }
