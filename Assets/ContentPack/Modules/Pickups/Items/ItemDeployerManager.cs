...
namespace TreasureTrove.Items
{
    public static class ItemDeployerManager
    {
        ...
        {
            int itemDeployCount = msr.leaderMasterInstance.inventory.GetItemCount(TTContent.Items.ItemDeployer);
            if (itemDeployCount > 0)
            {
                ...
                for (int i = maxItemsToGive - 1; i >= 0; i--)
                {
                    ...
                    if ((itemdef.ContainsTag(ItemTag.AIBlacklist) || itemdef.ContainsTag(ItemTag.CannotCopy)) || itemdef.hidden || itemdef == TTContent.Items.ItemDeployer)
                        continue;
